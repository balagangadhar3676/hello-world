...
void splitwords(char *str)
{
    ...
    char ptr[4][20] = {"HBT0", "DAT0", "HBT1", "DAT1"};
    int i = 0, j = 0, len = 0;
    __cnt = 0;
...
void *health_monitor(void *vargp)
{
    ...
    while (1)
    {
        ...
        {
            info_log(HEALTH_TAG, "[health_monitor]: second interface is false state:");
            __is_second_interface = false;
        }
        if ((strstr(avail_interface_list, "usb0")) == 0)
        {
            info_log(HEALTH_TAG, "[health_monitor]: usb interface is false state");
